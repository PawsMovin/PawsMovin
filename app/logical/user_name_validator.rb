...
class UserNameValidator < ActiveModel::EachValidator
  def validate_each(rec, attr, value)
    ...
    rec.errors.add(attr, "already exists") if User.find_by_normalized_name(name).present?
  ...
  def self.validate(record, attr = :name, duplicate: false)
    r = record.dup
    new(attributes: [attr]).validate(r)
    r.errors[attr] - (duplicate ? [] : ["already exists"])
  end
end
