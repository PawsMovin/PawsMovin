    ...
    class Pixiv < Base
      ...
      def remove_duplicates(sources)
        ...
        sources.delete_if do |source|
          url = begin
            Addressable::URI.heuristic_parse(source)
          ...
          if url.host == "www.pixiv.net" && url.path == "/member_illust.php" && url.query_values.present? && url.query_values["illust_id"].present?
            ...
            next true if $LAST_MATCH_INFO[:illust_id].to_i == our_illust_id
          elsif url.host == "www.pixiv.net" && url.path =~ %r{\A/en/artworks/(?<illust_id>\d+)\z}i # rubocop:disable Lint/DuplicateBranch
            next true if $LAST_MATCH_INFO[:illust_id].to_i == our_illust_id
          end
          false
        end
      ...
      def illust_id
        ...
        if url.host == "www.pixiv.net" && url.path == "/member_illust.php" && url.query_values.present? && url.query_values["illust_id"].present?
          ...
          return $LAST_MATCH_INFO[:illust_id].to_i
        ...
        elsif url.host =~ /\A(?:i\.pximg\.net|i\d+\.pixiv\.net)\z/i && # rubocop:disable Lint/DuplicateBranch
