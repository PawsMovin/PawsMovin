module PostVersionsHelper
  ...
  def post_version_diff(post_version)
    ...
    diff[:added_tags].each do |tag_name|
      ...
      changes << tag.ins(link_to("+#{tag_name}", show_or_new_wiki_pages_path(title: tag_name), class: classes))
    end
    diff[:removed_tags].each do |tag_name|
      classes = diff[:obsolete_removed_tags].include?(tag_name) ? "obsolete" : ""
      changes << tag.del(link_to("-#{tag_name}", show_or_new_wiki_pages_path(title: tag_name), class: classes))
    end
    diff[:unchanged_tags].each do |tag_name|
      changes << tag.span(link_to(tag_name, show_or_new_wiki_pages_path(title: tag_name)))
    end
  ...
  def post_version_locked_diff(post_version)
    ...
    diff[:added_locked_tags].each do |tag_name|
      changes << tag.ins(link_to("+#{tag_name}", show_or_new_wiki_pages_path(title: tag_name)))
    end
    diff[:removed_locked_tags].each do |tag_name|
      changes << tag.del(link_to("-#{tag_name}", show_or_new_wiki_pages_path(title: tag_name)))
    end
    diff[:unchanged_locked_tags].each do |tag_name|
      changes << tag.span(link_to(tag_name, show_or_new_wiki_pages_path(title: tag)))
    end
